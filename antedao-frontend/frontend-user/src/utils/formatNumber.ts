...
export const nFormatterMilion = (number: string, digits: any = 0) => {
  if(!number){ return "0"}
  const SI = [
    { value: 1, symbol: "" },
    { value: 1E6, symbol: "M" },
  ...
  return format((num / SI[i].value).toFixed(digits)).replace(rx, "$1") + SI[i].symbol;
}
...
export const roundShortNumberBuyDecimals = (balance: any, decimals: number = 4) => {
  ...
  if (balanceNumber.includes('.')) {
    ...
    const decimalsString = balanceSplit.pop();
    if (decimalsString.length > decimals) {
      return `${balanceSplit[0]}.${decimalsString.substring(0, 4)}`
    }
    return balance;
  }
...
export const divideTheExponent18 = (num: any) => {
  if (!num) return '0.00'
  return new BigNumber(num)
    .div(new BigNumber(10).exponentiatedBy(TOKEN_DECIMALS))
    .toFixed(0, BigNumber.ROUND_DOWN); // divide by token decimal
};