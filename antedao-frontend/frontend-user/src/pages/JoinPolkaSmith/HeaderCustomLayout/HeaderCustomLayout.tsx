        ...
        <div className={styles.navBar}>
          <div>
            <Link to={'/'} className={styles.navbarLink}>
              <img alt="Logo" src={logo} className={styles.navbarLogo}/>
            </Link>
          </div>
          {isWidthDown('xs', props.width) && <img alt="Open sidebar" src={iconHamburger} onClick={() => setOpenSideBar(true)}/>}
          <div className={styles.rightBar + (openSideBar ? ' active' : '')}>
            {isWidthDown('xs', props.width) &&
            <>
              <img alt="Logo" src={logo} className={styles.sideBarLogo}/>
              <img alt="Close" src={iconClose} className={styles.closeBtn} onClick={() => setOpenSideBar(false)}/></>}
