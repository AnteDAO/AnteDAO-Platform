...
const useUserPurchaseSignature = (connectedAccount: string | undefined | null, campaignId: number | undefined, authSignature: string | undefined, captchaToken: string | undefined) => {
  ...
  useEffect(() => {
        ...
        try {
          ...
          if (response.data && response.status && response.status === 200) {
            ...
            if (data && status === 200) {
              ...
              setMaxBuy(data.max_buy_decimal);
            }
    ...
    connectedAccount && campaignId && authSignature && captchaToken && getUserSignature();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [connectedAccount, campaignId, authSignature, captchaToken]);
