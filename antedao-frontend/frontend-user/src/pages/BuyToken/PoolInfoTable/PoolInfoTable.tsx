...
function PoolInfoTable(props: any) {
  ...
  return (
    <>
      {Object.keys(poolDetailsMapping)
        .filter(
          (key: string) =>
            poolDetails?.method === "whitelist" ||
            key !== PoolDetailKey.joinTime
        )
        .map((key: string) => {
          ...
          return (
            <div className={styles.poolDetailBasic} key={key}>
              <span className={styles.poolDetailBasicLabel}>
                {poolDetail.label}
              </span>
              <p className={styles.poolsDetailBasicText}>
                {poolDetail.image && (
                  <img
                    ...
                    className={styles.poolDetailBasicIcon}
                  />
                )}
                <Tooltip
                  title={<p style={{ fontSize: 15 }}>{poolDetail.display}</p>}
                >
                  <span>
                    {key !== PoolDetailKey.exchangeRate &&
                      key !== PoolDetailKey.usdtExchangeRate &&
                      poolDetail.display}
                  </span>
                </Tooltip>
                {poolDetail.utilIcon && (
