...
const StatusPendingIcon = () => {
	return (
		<Tooltip
			...
			title={
				<p style={{ fontSize: 12 }}>
					Pending: Checking is temporarily unavailable
				</p>
			}
		>
			<svg
	...
	return status !== undefined ? (
		<>
			{(status === 1 && <StatusCompletedIcon />) ||
				(status === 3 && <StatusRejectedIcon />) || <StatusPendingIcon />}
		</>
	) : (
		<></>
	);
	...
	return (
		<>
			<div className={styles.socialStep}>
				<p style={{ fontWeight: 700, fontSize: 14, lineHeight: "23.8px" }}>
					Follow and subscribe to {poolDetails?.title}'s and AnteDAO’s accounts
				</p>
			</div>
			...
			>
				{!isWidthDown("md", props.width) && (
					<table className={styles.socialFollowTable}>
						...
						<tbody>
							<tr  hidden={!poolDetails?.socialRequirement?.partner_twitter && !poolDetails?.socialRequirement?.self_twitter}>
							...
							<tr hidden={!poolDetails?.socialRequirement?.partner_channel && !poolDetails?.socialRequirement?.self_channel}>
								<td>Official Telegram</td>
								<td>
									<div className="flex-cell">
