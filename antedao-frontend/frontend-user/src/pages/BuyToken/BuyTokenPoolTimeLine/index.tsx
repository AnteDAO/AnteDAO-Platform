...
const BuyTokenPoolTimeLine: FC<Props> = ({ currentStatus, display, poolDetails, countDownDate }) => {
  ...
  const statusBarSteps: { name: string, value: string, active: string, tooltip: string }[] = [
    ...
    },
    {
      ...
      value: '3',
      active: 'Filled',
      tooltip: `Filled status is displayed when the pool reached its maximum swap value (100%).`,
    },
    {
      name: 'Claimable',
      value: '4',
    ...
    },
    {
      ...
      value: '5',
      active: 'Ended',
      tooltip: `The pool will end after users have claimed their tokens.`,
    },
  ]
              ...
              <li key={index} className={`${styles.itemStatusBarSteps} ${item.active === currentStatus ? 'active' : ''}`}>
                ...
                <Hidden smDown>
                  <Tooltip
                    ...
                    placement="bottom"
                  >
                    <span className={`${styles.itemValue} itemValue ${item.active === currentStatus ? 'active' : ''}`}>{item.value}</span>
                  </Tooltip>
                </Hidden>
                <span className={`${styles.itemName} itemName ${item.active === currentStatus ? 'active' : ''} `}>{item.name}</span>
              </li>
            )
          })
        }
