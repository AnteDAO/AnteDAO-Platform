...
const LinearPool = (props: any) => {
  ...
  const {approveToken , tokenApproveLoading, transactionHash: approveTransactionHash} = useTokenApprove(tokenDetails, connectedAccount, poolAddress, false, false);
  ...
  useEffect(()=>{
    retrieveTokenRawBalance(tokenDetails, connectedAccount)
      .then(balance => {
  ...
  const handleStake = async () =>{
    try {
      ...
      if (poolDetail?.stakingAmount !== '0' && poolDetail?.lockDuration !== '0' && confirmed === false) {
        ...
        setConfirmationText(`You currently have ANTE staked in this pool. If you want to stake more tokens, the expiry date will be extended to ${moment.unix(moment().unix() + Number(poolDetail?.lockDuration)).format("YYYY-MM-DD HH:mm:ss ([GMT]Z)")}`);
