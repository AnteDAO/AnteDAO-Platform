...
import { SIGN_REJECT_MESSAGE, TRANSACTION_ERROR_MESSAGE } from '../constants/alert';
        ...
        if (connector === ConnectorNames.WalletConnect) {
          ...
          await (provider as any).sendAsync({
            ...
            if (err || result.error) {
              ...
              const errCode = (err?.code || (err as any).error) || result.error.code
              console.log('Error when signing message: ', errCode,errMsg);
              setError(errMsg);
              if(errCode === 4001){
                dispatch(alertFailure(SIGN_REJECT_MESSAGE));
                return
              }
