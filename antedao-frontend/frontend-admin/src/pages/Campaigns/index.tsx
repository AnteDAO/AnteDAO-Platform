...
import CampaignsRecord from './CampaignsRecord'
import {adminRoute} from "../../utils";

const tableHeaders = ["CAMPAIGN NAME", "START TIME", "FINISH TIME",  "TOKEN SYMBOL", "STATUS"];
    ...
    <DefaultLayout>
      <div className={classes.header}>
        <div className="header-left">
          <Button to={adminRoute('/campaigns/add')} text={'Create New Campaign'} icon={'icon_plus.svg'} />
        </div>
      ...
      <div className={classes.refreshCampaigns}>
        <span className={classes.refreshCampaignsContainer}>
          <FormControlLabel
            ...
            label={<p className={classes.refreshCampaignsText}>Filter by your own campaigns</p>}
          />
        </span>
      ...
      <TableContainer component={Paper} className={classes.tableContainer}>
        ...
        )}
        {
          failure ? <p className={classes.errorMessage}>{failure}</p> : ((!campaigns || campaigns.length === 0) && !loading)  ? <p className={classes.noDataMessage}>There is no data</p> : (
            <>
              {campaigns && lastPage > 1 && <Pagination page={currentPage} className={classes.pagination} count={lastPage} onChange={handlePaginationChange} variant="outlined" shape="rounded" />}
            </>
          )
        }
      </TableContainer>
