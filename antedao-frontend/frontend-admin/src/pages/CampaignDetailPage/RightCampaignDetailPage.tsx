  ...
  return (
    <div className={styles.rightPage}>
      ...
      <TableContainer component={Paper} className={styles.tableTransactionList}>
        ...
        {
          failure ? <p className={styles.errorMessage}>{failure}</p> : ((!transactions || transactions.length === 0) && !loading)  ? <p className={styles.noDataMessage}>There is no data</p> : ( lastPage > 1 && <Pagination page={currentPage} className={styles.pagination} count={lastPage} onChange={handlePaginationChange} variant="outlined" shape="rounded" /> )
        }
      </TableContainer>
    </div>
  );
