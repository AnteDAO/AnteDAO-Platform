...
export const deployPool = (campaign: any, callback: (deployed: boolean) => void) => {
  return async (dispatch: ThunkDispatch<{}, {}, AnyAction>, getState: () => any) => {
    try {
      dispatch({ type: campaignActions.MY_CAMPAIGN_CREATE_REQUEST });

      const { start_time, finish_time, start_pre_order_time,
        token, address_receiver, token_by_eth, tokenInfo,
        accept_currency, network_available, total_sold_coin
      } = campaign;
      ...
      // native token
      const offeredCurrencyTokenAddress = MAPPING_CURRENCY_ADDRESS[network_available]?.[accept_currency] || NATIVE_TOKEN_ADDRESS
      const offeredCurrencyTokenInfo = await getTokenInfo(offeredCurrencyTokenAddress);
      const offeredCurrencyDecimals = offeredCurrencyTokenInfo?.decimals || 0;
      const offeredRate = new BigNumber(1).dividedBy(token_by_eth).multipliedBy(Math.pow(10,tokenInfo.decimals)).multipliedBy(Math.pow(10,18)).toFixed(0);

      let factorySmartContract = getContractInstance(campaignFactoryClaimABI, process.env.REACT_APP_SMART_CONTRACT_PRESALE_FACTORY_ADDRESS || '');

      switch (network_available) {
        case NETWORK_AVAILABLE.BSC:
          factorySmartContract = getContractInstance(campaignFactoryClaimABI, process.env.REACT_APP_SMART_CONTRACT_BSC_PRESALE_FACTORY_ADDRESS || '', false);
          break
        case NETWORK_AVAILABLE.POLYGON:
          factorySmartContract = getContractInstance(campaignFactoryClaimABI, process.env.REACT_APP_SMART_CONTRACT_POLYGON_PRESALE_FACTORY_ADDRESS || '', false);
          break
        default:
      }
      if (factorySmartContract) {
        ...
        const createdCampaign = await factorySmartContract.methods.registerPool(
          token,
          new BigNumber(total_sold_coin).multipliedBy(Math.pow(10, tokenInfo.decimals)).toFixed(),
          durationTime,
          startTimeUnix,
          offeredCurrencyTokenAddress,
        ...
        if (createdCampaign) {
          ...
          const isUpdateDeploy = await updateDeploySuccess(updateData, campaign.id);
          if(isUpdateDeploy?.status === 400){
            dispatch({ type: alertActions.ERROR_MESSAGE, payload: isUpdateDeploy?.message });
          }else{
...
export const getOwnerWalletAddress = async (network_available: string) => {
  try {
    let factorySmartContract = getContractInstance(campaignFactoryClaimABI, process.env.REACT_APP_SMART_CONTRACT_PRESALE_FACTORY_ADDRESS || '');
    ...
    switch (network_available) {
      ...
      case NETWORK_AVAILABLE.POLYGON:
        factorySmartContract = getContractInstance(campaignFactoryClaimABI, process.env.REACT_APP_SMART_CONTRACT_POLYGON_PRESALE_FACTORY_ADDRESS || '', false);
        break;
        case NETWORK_AVAILABLE.POLYGON_TESTNET:
          factorySmartContract = getContractInstance(campaignFactoryClaimABI, process.env.REACT_APP_SMART_CONTRACT_POLYGON_PRESALE_FACTORY_ADDRESS || '', false);
          break
      default:
    }
    if (factorySmartContract) {
      const owerWalletAddress = await factorySmartContract.methods.owner().call();
      return owerWalletAddress;
    }
    // throw ("Can't get Owner Wallet Address");
  } catch (err: any) {
    return "";
  }
} 