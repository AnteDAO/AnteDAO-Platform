...
export const updateStakingPoolFee = (data: any, setDisable:any, setLoadingUpdate:any) => {
  return async (dispatch: ThunkDispatch<{}, {}, AnyAction>, getState: () => any) => {
    try {
      let stakingPoolContract = getContractInstance(stakingPoolABI, SP_ADDRESS || '');
      setDisable(true)
      if (!stakingPoolContract) {
        dispatch({
          type: alertActions.ERROR_MESSAGE,
      ...
      const userWalletAddress = getState().user.data.wallet_address;
      if (data && userWalletAddress && stakingPoolContract) {
        const { stake, unstake, fee,address } = data
        await stakingPoolContract.methods.updateFeeAndStatus(stake, unstake, new BigNum(fee).multipliedBy(100).toString(),address).send({ from: userWalletAddress })
