...
class UserController {
  ...
  *       - name: searchQuery
  *         description: searchQuery(wallet_address,email,or username)
  *         in: query
  *         required: false
  *         type: string
  ...
  */
  ...
  async userList({ request }) {
    try {
      ...
      let cachedUserQuery = CachedUserModel.query()
        .where((q) => {
          if (walletAddress)
            q.where("wallet_address", "like", `%${walletAddress}%`);
  ...
  *     responses:
  *       200:
  *         description:  user un  join campain
  *         example:
  */
    ...
    try {
      ...
      if (await RedisUtils.checkExistRedisUserTierBalance(wallet_address)) {
        const cached = JSON.parse(
          await RedisUtils.getRedisUserTierBalance(wallet_address)
        );
  ...
  async profile({ request }) {
    try {
      const params = request.all();
      const wallet_address = params.wallet_address;
      const findedUser = await UserModel.query()
  ...
  async updateProfile({ request, auth }) {
    try {
      ...
      const user = await userService
        .buildQueryBuilder({ wallet_address })
        .first();
      if (!user) {
  ...
  async uploadAvatar({ request }) {
    ...
    const fileName =
      timeStamp + "_" + (profilePic.clientName || "").replace(/\s/g, "_");
  ...
  async resetPassword({ request, auth }) {
    try {
      ...
      if (e.status === 400) {
        ...
        return HelperUtils.responseErrorInternal(
          "Server Error: Reset password fail"
        );
      }
    }
  }
  ...
  async confirmEmail({ request }) {
    try {
      ...
      if (!checkToken) {
        return HelperUtils.responseErrorInternal(
          "Active account link has expried."
        );
      }
  ...
  async changeType({ request }) {
    try {
      ...
      if (param.basic_token != process.env.JWT_BASIC_AUTH) {
        return ErrorFactory.unauthorizedInputException(
          "Basic token error!",
          "401"
        );
      }
      ...
      const type =
        param.type == Const.USER_TYPE.WHITELISTED
  ...
  /**
  * @swagger
  * /pool/{campaignId}/user/{walletAddress}/current-tier:
  *   get:
  *     tags:
  *       - User 
  *     summary: get user tier  and max buy min buy in this campaign
  *     parameters:
  *       - name: wallet_address
  *         description: wallet_address of user
  *         in: query
  ...
  *     responses:
  *       200:
  *         description: tier is fixed(when join pool)
  *         example:
  */
  ...
  async getCurrentTier({ request, params }) {
    try {
      ...
      // console.log(
      //   "[getCurrentTier] - poolExist.public_winner_status:",
      //   poolExist && poolExist.public_winner_status
      // );
      ...
      if (await RedisUtils.checkExistRedisUserTierBalance(walletAddress)) {
        const cached = JSON.parse(
          await RedisUtils.getRedisUserTierBalance(walletAddress)
        );
        const tierCacheTimeToLive = 5 * 60 * 1000; // 5 minutes
      ...
      const camp = (
        await CampaignModel.query().where("id", campaignId).fetch()
      ).toJSON();
      ...
      // console.log("2--------------------------");
      var maxAllocation = await campaignService.getUserMaxBuy(
        walletAddress,
        campaignId
      );
      ...
      // console.log("3--------------------------");
      // check token remain
      const tokenRemain = await campaignService.getTokenRemainOfCampain(
        camp[0]
      );
  ...
  *     parameters:
  *       - name: wallet_address
  *         description: wallet address input
  *         in: query
  *         required: false
  *         type: string
  *       - name: email
  ...
  *     responses:
  *       200:
  *         description: active kyc success
  *         example:
  */
  async activeKyc({ request, params }) {
    const inputParams = request.only(["wallet_address", "email"]);
    try {
      const userService = new UserService();
      ...
      if (!userFound.is_kyc) {
        const user = await userService
          .buildQueryBuilder({ id: userFound.id })
          .update({ is_kyc: Const.KYC_STATUS.APPROVED });
  ...
  async kycUpdateStatus({ request }) {
    ...
    try {
      ...
      const url = process.env.BLOCK_PASS_API_URL.replace(
        "CLIENT_ID",
        process.env.BLOCK_PASS_CLIENT_ID
      ...
      if (!response || response.statusCode !== 200) {
        // console.log(
        //   `Failed when call block pass api with refID ${params.refId} and ${params.recordId}`
        // );
      ...
      const wallet = JSON.parse(response.body).data.identities
        .crypto_address_matic.value;
      const kycStatus = JSON.parse(response.body).data.status;

      const address_country = JSON.parse(
        JSON.parse(response.body).data.identities.address.value
      ).country;
      let passport_issuing_country = address_country;

      if (
        JSON.parse(response.body).data.identities.passport_issuing_country !=
        null
      ) {
        passport_issuing_country = JSON.parse(response.body).data.identities
          .passport_issuing_country.value;
      } else if (
        JSON.parse(response.body).data.identities.national_id_issuing_country !=
        null
      ) {
        passport_issuing_country = JSON.parse(response.body).data.identities
          .national_id_issuing_country.value;
      } else if (
        JSON.parse(response.body).data.driving_license_issuing_country != null
      ) {
        passport_issuing_country = JSON.parse(response.body).data.identities
          .driving_license_issuing_country.value;
      }
      ...
      if (
        Const.KYC_STATUS[kycStatus.toString().toUpperCase()] ==
        Const.KYC_STATUS.APPROVED
      ) {
        const approvedRecord = await BlockpassApprovedModel.query()
      ...
      if (!email || !wallet) {
        // console.log(
        //   `Do not found user with email ${email} and wallet ${wallet}`
        // );
        return HelperUtils.responseBadRequest();
      }
  ...
  async getEPkfBonusBalance({ request }) {
    ...
    const config = await ConfigModel.query()
      .where("key", wallet_address)
      .first();

    return {
      code: 200,
      message: "Success",
      data: new BigNumber((config && config.value) || 0)
  ...
  async kycUserUpdate({ request, params }) {
    try {
      ...
      await userService
        .buildQueryBuilder({ id: params.id })
        .update(updateInputs);
