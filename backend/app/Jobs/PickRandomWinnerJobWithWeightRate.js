...
class PickRandomWinnerJobWithWeightRate {
  ...
  static async handle(data) {
    // console.log('PickRandomWinnerJobWithWeightRate-job started', data);
    try {
      // do snapshot balance
      await PickRandomWinnerJobWithWeightRate.doSnapshotBalance(data);
      // pickup random winner after snapshot all whitelist user balance
      await PickRandomWinnerJobWithWeightRate.doPickupRandomWinner(data);
    } catch (e) {
  ...
  static async doPickupRandomWinner(data) {
    ...
    for (let i = 0; i < data.tiers.length; i++) {
      ...
      if (tier.level == 0) {
        ...
        winners = await PickRandomWinnerJobWithWeightRate.pickupRandom(userSnapshots.toJSON(), tier.ticket_allow, data.campaign_id);
      } else {
...
module.exports = PickRandomWinnerJobWithWeightRate
