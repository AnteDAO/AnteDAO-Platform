...
class PoolService {
  ...
  async getPoolsUserAppliedWhitelist(walletAddress, params) {
    const query = CampaignModel.query('*').from('campaigns')
                    .leftJoin('whitelist_submissions', 'campaigns.id', 'whitelist_submissions.campaign_id')
                    .where('whitelist_submissions.wallet_address', walletAddress);
    if (params.type == Const.POOL_IS_PRIVATE.PUBLIC ||
      params.type == Const.POOL_IS_PRIVATE.PRIVATE ||
      params.type == Const.POOL_IS_PRIVATE.SEED ||
      params.type == Const.POOL_IS_PRIVATE.COMMUNITY) {
  ...
  async getActivePoolsV3(filterParams) {
    ...
    let pools = await this.buildQueryBuilder(filterParams).select('campaigns.*')
      ...
      .orderBy('campaigns.start_time', 'DESC')
      .paginate(page, limit);
  ...
  async runUpdatePoolStatus() {
    ...
    // console.log("lisst poolllll", pools)
