...
const SOCIAL_CHECK_LINK = process.env.SOCIAL_CHECK_LINK || 'https://AnteDao-social-api.polkafoundry.com/api/v1/social-network/NETWORK/TYPE';
...
class WhitelistSubmissionService {
  ...
  async updateUserPurchasedAndUserClaimed(walletAddress, campaignId, userPurchased, userClaimed) {
    await WhitelistSubmissionModel
      .query()
      .where('wallet_address', '=', walletAddress)
      .andWhere('campaign_id', '=', campaignId)
      .update({ user_purchased: userPurchased, user_claimed: userClaimed});
  }
