...
const STAKING_CONTRACT_CONFIGS =
  NETWORK_CONFIGS.contracts.StakingPool;
const  STAKING_POOL_CONTRACT_ABI =
  STAKING_CONTRACT_CONFIGS.CONTRACT_DATA;

const SBX_SMART_CONTRACT_ADDRESS = process.env.SBX_SMART_CONTRACT_ADDRESS;
const UNI_LP_PKF_SMART_CONTRACT_ADDRESS =
  process.env.UNI_LP_PKF_SMART_CONTRACT_ADDRESS;
...
const START_TIME_TRANSFER_STAKING_CONTRACT =
  Const.START_TIME_TRANSFER_STAKING_CONTRACT;
const END_TIME_TRANSFER_STAKING_CONTRACT =
  Const.END_TIME_TRANSFER_STAKING_CONTRACT;
...
const getWeb3PolygonProviderLink = (isChangeRPC = false) => {
  if (isDevelopment) {
    ...
    if (isChangeRPC) {
      ...
      // console.log(
      //   `=======> Change RPC URL to ${web3ApiUrlIndex} - ${WEB3_API_URLS[web3ApiUrlIndex]}`
      // );
    }
...
const getPolygonNetwork = (props) => {
  return new Web3(getWeb3ProviderLink(props.isChangeRPC));
};

// Tier Smart contract
const {
  abi: CONTRACT_TIER_ABI,
} = require("../../blockchain_configs/contracts/Normal/Tier.json");
const TIER_SMART_CONTRACT = process.env.TIER_SMART_CONTRACT;
const {
  abi: CONTRACT_STAKE_ABI,
} = require("../../blockchain_configs/contracts/Normal/MantraStake.json");
const {
  abi: CONTRACT_ERC20_ABI,
} = require("../../blockchain_configs/contracts/Normal/Erc20.json");
const MANTRA_DAO_STAKE_SMART_CONTRACT =
  process.env.MATRA_DAO_STAKE_SMART_CONTRACT;
const ETH_SMART_CONTRACT_USDT_ADDRESS =
  process.env.ETH_SMART_CONTRACT_USDT_ADDRESS;
const ETH_SMART_CONTRACT_USDC_ADDRESS =
  process.env.ETH_SMART_CONTRACT_USDC_ADDRESS;
const BSC_SMART_CONTRACT_USDT_ADDRESS =
  process.env.BSC_SMART_CONTRACT_USDT_ADDRESS;
const BSC_SMART_CONTRACT_USDC_ADDRESS =
  process.env.BSC_SMART_CONTRACT_USDC_ADDRESS;
const BSC_SMART_CONTRACT_BUSD_ADDRESS =
  process.env.BSC_SMART_CONTRACT_BUSD_ADDRESS;
const EPKF_BONUS_LINK = process.env.EPKF_BONUS_LINK || "";
const POLYGON_SMART_CONTRACT_USDT_ADDRESS =
  process.env.POLYGON_SMART_CONTRACT_USDT_ADDRESS;
const POLYGON_SMART_CONTRACT_USDC_ADDRESS =
  process.env.POLYGON_SMART_CONTRACT_USDC_ADDRESS;
...
const checkRole = (params, extraData) => {
  return {
    ...params,
    role:
      params.type === Const.USER_TYPE_PREFIX.ICO_OWNER
        ? Const.USER_ROLE.ICO_OWNER
        : Const.USER_ROLE.PUBLIC_USER,
  };
};
...
const responseBadRequest = (message) => {
  return {
    ...
    message:
      message ||
      "Looks like this is unkown request, please try again or contact us.",
    data: null,
  };
};
...
const getStakingPoolsSmartContractInstance = () => {
  const stakingPoolSC = new web3.eth.Contract(
    STAKING_POOL_CONTRACT_ABI,
    STAKING_POOL_SMART_CONTRACT
  );
  return stakingPoolSC;
};

const getMantraStakeSmartContractInstance = () => {
  const mantraSc = new web3.eth.Contract(
    CONTRACT_STAKE_ABI,
    MANTRA_DAO_STAKE_SMART_CONTRACT
  );
  return mantraSc;
};
...
const getEPkfBonusBalance = (wallet_address) => {
  return axios
    .get(EPKF_BONUS_LINK.replace("WALLET_ADDRESS", wallet_address))
    .catch((e) => {
      return {};
    });
};
...
const getTiers = async () => {
  const tierSettings = (
    await TierSettingModel.query().orderBy("tier", "asc").fetch()
  ).toJSON();
  return tierSettings.map((tierSetting) =>
    Web3.utils.toWei(tierSetting.token_amount.toString())
  );
};

const getCampaignContract = async (network_available, token) => {
  const stakingPoolSC = new networkToWeb3[network_available].eth.Contract(
    CONTRACT_ABI,
    token
  );
  return stakingPoolSC;
...
const getStakingPoolSbx = async (wallet_address) => {
  const pools = await StakingPoolModel.query()
    .where("rkp_rate", ">", 0)
    .fetch();
...
const getUserTierSmart = async (wallet_address) => {
  ...
  let stakedSbx = new BigNumber(
    (receivedData[1] && receivedData[1].stakedSbx) || 0
  );
  ...
  // console.log(
  //   "[getUserTierSmart] wallet_address - userTier: ",
  //   wallet_address,
  //   userTier
  // );
  // console.log("[getUserTierSmart] sbxEq", sbxEq.toFixed());
...
const getManyUserTierSmart = async (walletAddressArr) => {
  ...
  const multicalGetStakingData = async (props) => {
    try {
      const multicall = new ethereumMulticall.Multicall({
        web3Instance: getPolygonNetwork({ isChangeRPC: props.isChangeRPC }),
        tryAggregate: true,
      });

      const contractCallContext = [];
      // console.log('STAKING_POOL_SMART_CONTRACT: ',STAKING_POOL_SMART_CONTRACT)
      contractCallContext.push({
        ...
        calls: [
          {
            reference: "acceptedTokenCall",
            methodName: "linearAcceptedToken",
            methodParameters: [],
          },
        ],
      });
      ...
      for (let i = 0; i < walletAddressArr.length; i++) {
        const address= checkSumAddress(walletAddressArr[i])
        contractCallContext.push(
          ...listPool.map((pool) => {
            return {
              ...
              calls: [
                {
                  reference: `pool_${pool.pool_id}`,
                  methodName: "linearStakingData",
                  methodParameters: [pool.pool_id, address],
                },
              ],
            };
          })
      ...
      // console.log(
      //   "[getStakingPoolSbx] Get staking data successfully *****************"
      // );
      return result;
    } catch (error) {
      console.log(
  ...
  try {
    ...
    // console.log("result multical: ---------------------------------", result);
    for (let i = 0; i < walletAddressArr.length; i++) {
    ...
    for (const [key, value] of Object.entries(result.results)) {
      if (key === "acceptedToken") {
        if(!value.callsReturnContext[0].returnValues[0]){
          break;
        }
        if (
          value.callsReturnContext[0].returnValues[0].toLowerCase() !==
          SBX_SMART_CONTRACT_ADDRESS.toLowerCase()
        ) {
          break;
        }
      } else {
    ...
    console.log(
      "[getStakingPoolSbx] Get data successfully - userStakedSbxArr: "
    );
...
const getExternalTokenSmartContract = async (wallet_address) => {
  ...
  const externalTokenMantra = await tierSc.methods
    .externalToken(MANTRA_DAO_STAKE_SMART_CONTRACT)
    .call();
  // console.log(
  //   "[getExternalTokenSmartContract] - externalToken",
  //   externalTokenMantra
  // );
...
const getUserTotalStakeSmartContract = async (wallet_address) => {
  const tierSc = getTierSmartContractInstance();
  const totalStaked = await tierSc.methods
    .userTotalStaked(wallet_address)
    .call();
...
const getUnstakeMantraSmartContract = async (wallet_address) => {
  ...
  const unstakeAmountMantra = await mantraSc.methods
    .getUnstake(wallet_address)
    .call();
  // console.log(
  //   "[getUnstakeMantraSmartContract] - unstakeAmountMantra",
  //   unstakeAmountMantra
  // );
...
const getContractInstanceDev = async (camp) => {
  ...
  return new networkToWeb3Dev[camp.network_available].eth.Contract(
    getABIbyCampaign(camp),
    camp.campaign_hash
  );
};
...
const getContractInstance = async (camp) => {
  if (isDevelopment) {
    // Prevent limit request Infura when dev
    return getContractInstanceDev(camp);
  }
  return new networkToWeb3[camp.network_available].eth.Contract(
    getABIbyCampaign(camp),
    camp.campaign_hash
  );
};
...
const getTokenSoldSmartContract = async (pool) => {
  ...
  const poolContract = isClaimable
    ? await getContractClaimInstance(pool)
    : await getContractInstance(pool);
...
const getOfferCurrencyInfo = async (camp) => {
  ...
  switch (camp.accept_currency) {
    ...
    case Const.ACCEPT_CURRENCY.USDC:
      ...
      scCurrency =
        networkCurrencyToContract[camp.accept_currency][camp.network_available]
          .scCurrency;
      unit =
        networkCurrencyToContract[camp.accept_currency][camp.network_available]
          .uint;
    ...
    default:
      // console.log(
      //   `Do not found currency support ${camp.accept_currency} of campaignId ${camp.id}`
      // );
...
const getSymbolByTokenAddress = async ({
  network = Const.NETWORK_AVAILABLE.ETH,
  address,
}) => {
  const contractToken = new networkToWeb3[network].eth.Contract(
    CONTRACT_ERC20_ABI,
    address
  );
...
const getLastClaimConfig = (poolDetails) => {
  if (
    ...
    poolDetails.campaignClaimConfig.length > 0
  ) {
    const lastClaim =
      poolDetails.campaignClaimConfig[
...
const getLastClaimConfigTime = (poolDetails) => {
  ...
  if (lastClaim) {
    const startClaim = new BigNumber(lastClaim.start_time || 0)
      .plus(7 * 24 * 3600)
      .toFixed(); // +1week
    return startClaim;
  }
...
const getLastActualFinishTime = (poolDetails) => {
  if (poolDetails.finish_time) {
    const actualFinishTime = new BigNumber(poolDetails.finish_time || 0)
      .plus(12*60*60)
      .toFixed(); // +12h
...
const getPoolStatusByPoolDetail = async (poolDetails, tokenSold) => {
  ...
  const startBuyTime = startBuyTimeField()
    ? new Date(Number(startBuyTimeField()) * 1000)
    : undefined;
  const endBuyTime = endBuyTimeField()
    ? new Date(Number(endBuyTimeField()) * 1000)
    : undefined;
  const startJoinTime = startJoinTimeField()
    ? new Date(Number(startJoinTimeField()) * 1000)
    : undefined;
  const endJoinTime = endJoinTimeField()
    ? new Date(Number(endJoinTimeField()) * 1000)
    : undefined;
  const releaseTime = releaseTimeField()
    ? new Date(Number(releaseTimeField()) * 1000)
    : undefined;
  ...
  let { progress } = getProgressWithPools({
    ...poolDetails,
    tokenSold:
      tokenSold || poolDetails.tokenSold || poolDetails.token_sold || "0",
  });
  ...
  if (poolDetails && poolDetails.campaign_hash) {
    CampaignContract = await getCampaignContract(
      poolDetails.network_available,
      poolDetails.campaign_hash
    );
    // total token user is unClaimed
    const totalUnclaimed = await CampaignContract.methods
      .totalUnclaimed()
      .call();
    // console.log("totalUnclaimed: ", totalUnclaimed);

    // total token user is bought
    const totalSoldCoin = poolDetails.total_sold_coin;

    // claim all token
    if (totalUnclaimed === 0) {
      return PoolStatus.CLOSED;
    }
    if (totalUnclaimed === totalSoldCoin) {
      return PoolStatus.CLAIMABLE;
    }
  }
...
const getDecimalsByTokenAddress = async ({
  network = Const.NETWORK_AVAILABLE.ETH,
  address,
}) => {
  const contractToken = new networkToWeb3[network].eth.Contract(
...
const checkIsInPreOrderTime = (poolDetails, currentUserTierLevel) => {
  ...
  let startPreOrderTime =
    poolDetails.startPreOrderTime || poolDetails.start_pre_order_time;
...
const calcReputationPoint = {
  getPoint({ amount, stakedAt, ratePKF, currentTime = Date.now() }) {
    ...
    return new BigNumber(stakedAge)
      .times(new BigNumber(amount))
      .times(new BigNumber(percent))
      .times(new BigNumber(ratePKF))
      .dividedBy(100);
  },
  ...
  getPercentReputation(stakedDay) {
    ...
    return dayToPercent[
      Object.keys(dayToPercent)
        .map((day) => +day)
        .sort((a, b) => b - a)
        .find((day) => stakedDay >= day)
    ];
  },
  async getRKPData({ walletAddress, isCaching = false }) {
    if (
      !isCaching &&
      (await RedisUtils.checkExistRedisUserReputation(walletAddress))
    ) {
      const cached = JSON.parse(
        await RedisUtils.getRedisUserReputation(walletAddress)
      );

      const rkpCacheTimeToLive =
        process.env.NODE_ENV == "development"
          ? 15 * 60 * 1000
          : 13 * 60 * 60 * 1000; // dev: 15 minutes, prod - 13 hours
    ...
    const [reputationLog, rateSetting, rkpFromKSM, contractTier] =
      await Promise.all([
        ReputationLogModel.query()
          .where("wallet_address", checkSumAddress(walletAddress))
          .first(),
        RateSetting.query().first(),
        new Promise(async (resolve, reject) => {
          try {
            const result = await getEPkfBonusBalance(
    ...
    const stakingHistory = reputationLog
      ? JSON.parse(reputationLog.staking_history)
      : {
          oldStaked: [],
          allocStaked: [],
          linearStaked: [],
    ...
    const tokens = [
      ...new Set(
        Object.values(stakingHistory).reduce((arr, poolHis) => {
          return [...arr, ...poolHis.map((his) => his.token)];
        }, [])
      ),
    ];
    ...
    const [addressTokenToSymbol, oldStakedAmount] = await Promise.all([
      new Promise(async (resolve, reject) => {
        try {
          const tokensToSymbol = await Promise.all(
            tokens.map((token) => getSymbolByTokenAddress({ address: token }))
          );
          return resolve(
            tokensToSymbol.reduce((obj, symbol) => {
              return { ...obj, ...symbol };
            }, {})
          );
        } catch (error) {
        ...
        return {
          ...
          unstaked: his.unstaked.map((unstaked) => {
            return {
              ...
              remainingAmount: new BigNumber(unstaked.unstakedAmount)
                .minus(new BigNumber(unstaked.calculatedAmount))
                .toNumber(),
            };
          }),
          balance: his.balance,
          ratePKF,
          percent: this.getPercentReputation(
            this.getStakedDay({ stakedAt: his.stakedAt })
          ),
          days: this.getStakedDay({
            currentTime: isOldStaked
              ? Math.min(START_TIME_TRANSFER_STAKING_CONTRACT, Date.now())
              : Date.now(),
    ...
    const inTransferTime =
      Date.now() < START_TIME_TRANSFER_STAKING_CONTRACT
        ? -1
        : Date.now() > END_TIME_TRANSFER_STAKING_CONTRACT
        ? 1
        : 0;

    let { oldStakedHistories, newStakedHistories } = {
      oldStakedHistories: stakingHistory.oldStaked.map(
        formatStakedData({ isOldStaked: true })
      ),
      newStakedHistories: [
        ...stakingHistory.allocStaked,
        ...stakingHistory.linearStaked,
      ]
        .sort((a, b) => {
          return a.stakedAt - b.stakedAt;
        })
        .map(formatStakedData({ isOldStaked: false })),
    };

    if (
      inTransferTime == -1 ||
      (inTransferTime == 0 && oldStakedAmount !== "0")
    )
      newStakedHistories = [];
    ...
    const totalOldStaked = oldStakedHistories
      .filter((his) => his.balance)
      .reduce((total, his) => {
        return total.plus(
          new BigNumber(his.balance).times(
            new BigNumber(stakedTokenToRateRKP[checkSumAddress(his.token)] || 0)
          )
        );
      }, new BigNumber(0));

    const totalNewStaked = newStakedHistories
      .filter((his) => his.balance)
      .reduce((total, his) => {
        return total.plus(
          new BigNumber(his.balance).times(
    ...
    if (
      (inTransferTime == 0 &&
        ...
        !this.checkTotalStaked({
          oldStaked: totalOldStaked,
          newStaked: totalNewStaked,
        })) ||
      (inTransferTime == 1 &&
        !this.checkTotalStaked({
          oldStaked: totalOldStaked,
      ...
      oldStakedHistories = oldStakedHistories.map((his) => {
        return { ...his, points: 0 };
      });
    }
    ...
    if (
      new BigNumber(Date.now())
        .minus(new BigNumber(END_TIME_TRANSFER_STAKING_CONTRACT))
        .dividedBy(new BigNumber(TIME_REPUTATION_AGE).times(1000))
        .gte(45)
    )
      oldStakedHistories = [];

    const rkpFromOldStaked = oldStakedHistories
      .reduce(calcTotalPoints, new BigNumber(0))
      .toNumber();
    const rkpFromNewStaked = newStakedHistories
      .reduce(calcTotalPoints, new BigNumber(0))
      .toNumber();

    const rkpBonus =
      reputationLog && reputationLog.bonus ? reputationLog.bonus : 0;
    ...
    return {
      ...
      rkpFromKSM,
      rkpBonus,
      totalRKP: new BigNumber(rkpFromOldStaked)
        .plus(new BigNumber(rkpFromNewStaked))
        .plus(new BigNumber(rkpFromKSM))
        .plus(new BigNumber(rkpBonus))
        .toNumber(),
      stakingHistory: {
        oldStakedHistories,
        newStakedHistories,
  ...
  convertTokenToRateRKP(rateSetting) {
    return {
      [checkSumAddress(UNI_LP_PKF_SMART_CONTRACT_ADDRESS)]:
        (rateSetting && +rateSetting.lp_pkf_rate) || 1,
      [checkSumAddress(SBX_SMART_CONTRACT_ADDRESS)]: 1,
    };
  },
  checkTotalStaked({ oldStaked, newStaked }) {
    // true - keeping, fale - skip
    return newStaked.gte(oldStaked)
      ? true
      : oldStaked.minus(newStaked).lte(new BigNumber(Math.pow(10, 0)));
  },
};
...
const pushOnesignalNotification = async (body, playerIds) => {
  ...
  const result = await axios.post(Const.ONESIGNAL_NOTIFICATION_API, data, {
    headers: headers,
  });
  return {
